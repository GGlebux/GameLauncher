...
class Tetris:
    def __init__(self, bx, by):
        ...
        self.resx, self.resy = self.screen.get_size()
        ...
        # Кнопка выхода
        self.exit_button = pygame.Rect(self.resx - 100, 5, 80, 30)
        ...
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN:
                if self.exit_button.collidepoint(ev.pos):
                    pygame.quit()
                    from Menu import GradientMenu  # Импорт внутри обработчика
                    GradientMenu().run()  # Запуск класса GradientMenu из test.py
                    exit()
    ...
    def draw_game(self):
        ...
        pygame.draw.rect(self.screen, WHITE, self.exit_button)  # Кнопка белая
        exit_text = self.myfont.render("Exit", True, BLACK)  # Текст черный
